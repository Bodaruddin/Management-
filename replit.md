...
- Required runtime variable: `DATABASE_URL` — Replit-managed PostgreSQL connection
...
- The development app uses Replit's managed PostgreSQL database through the runtime-provided `DATABASE_URL`.
- `RENDER_DATABASE_URL` and `APP_DATABASE_URL` remain supported for external hosted deployments.
...
- Keep the mobile app connected to the managed PostgreSQL-backed API.
